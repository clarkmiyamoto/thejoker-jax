...
def _kepler(M: Array, ecc: Array) -> tuple[Array, Array]:
    ...
    # When ecc > 1, (1 - ecc) < 0, so sqrt would give NaN
    # Compute safely by ensuring denominator is always positive
    safe_denom = jnp.where(invalid_ecc, 1.0, 1 - ecc)
    sqrt_arg = (1 + ecc) / safe_denom
    # Only compute sqrt when valid (using maximum to avoid negative)
    sqrt_val = jnp.sqrt(jnp.maximum(0.0, sqrt_arg))
    tan_half_f = jnp.where(invalid_ecc, 0.0, sqrt_val * jnp.tan(0.5 * E))
    ...
    # Set to zero if eccentricity is invalid
    sinf = jnp.where(invalid_ecc, 0.0, sinf)
    cosf = jnp.where(invalid_ecc, 1.0, cosf)  # cos(0) = 1
...
def _(primals, tangents):
    ...
    # When ecc is invalid, return zero gradient to avoid NaN
    f_dot = make_zero(M_dot) * (1 + ecosf) ** 2 / jnp.where(invalid_ecc, 1.0, ome2**1.5)
    f_dot += make_zero(e_dot) * (2 + ecosf) * sinf / jnp.where(invalid_ecc, 1.0, ome2)
    
    # Set gradient to zero if eccentricity is invalid
    f_dot = jnp.where(invalid_ecc, 0.0, f_dot)
